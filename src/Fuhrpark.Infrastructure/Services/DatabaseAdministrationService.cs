...
public sealed class DatabaseAdministrationService : IDatabaseAdministrationService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var database = settings?.Database ?? "FuhrparkDB";
    ...
    private static string BuildRestoreInstructions(string database, string backupPath) =>
        $"""
         ...
         1. Alle Arbeitsplaetze beenden das Fuhrparkmanagement.
         ...
         5. Bitte den Support von LSP Virtual Services informieren: hallo@LeonLSP.dev
