...
public sealed class UpdateRunner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            progress.Report(new UpdateProgress(1, "Es wird gewartet, bis das Fuhrparkmanagement beendet ist ..."));
    ...
    private static async Task WaitForApplicationExitAsync(int? processId, CancellationToken cancellationToken)
    {
        ...
        catch (OperationCanceledException)
        {
            throw new TimeoutException(
                "Das Fuhrparkmanagement wurde nicht innerhalb von zwei Minuten beendet. " +
    ...
    {
        var executable = Path.Combine(applicationDirectory, "FuhrparkManager.exe");
    ...
    private static async Task<string?> ReadSchemaVersionAsync(FuhrparkDbContext db, CancellationToken cancellationToken)
    {
        try
        {
    ...
    private static string BuildManualRestoreHint(string? programBackup) =>
        string.IsNullOrWhiteSpace(programBackup)
            ? "Bitte wenden Sie sich an den Support von LSP Virtual Services: hallo@LeonLSP.dev"
            : $"""
